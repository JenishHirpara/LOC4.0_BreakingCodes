...
export default function SortingSelecting(props) {
  ...
  const { data } = props;
  function createData(type, axisCode, weight) {
    return { type, axisCode, weight };
  }

  // console.log(code);
  // console.log(COMPANY_DATA);

  const template = TEMPLATE_DATA['esg-balanced-1'];
  const templateData = {};

  Object.values(template.template).forEach((x) => {
    templateData[x.axisCode] = {};
    templateData[x.axisCode].weight = x.weight;
    templateData[x.axisCode].axisCode = x.axisCode;
    // esgScore += company[x.axisCode] * x.weight;
  });

  SORTING_SELECTING_TABLE = [
    createData('Environment', templateData['e-component-score'].axisCode, data.env),
    createData('Social', templateData['s-component-score'].axisCode, data.soc),
    createData('Governance', templateData['g-component-score'].axisCode, data.gov)
  ];
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.axisCode}</TableCell>
