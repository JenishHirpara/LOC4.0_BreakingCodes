...
export default function SortingSelecting(props) {
  ...
  function createData(type, name, value, score, risk) {
    return { type, name, value, score, risk };
  }
  ...
  const countryData = props.countryData.map((el) =>
    createData('Company', el.country, el.count, el['avg-esgscore'], getRisk(el['avg-esgscore']))
  );

  const sectorData = props.sectorData.map((el) =>
    createData('Industry', el.sector, el.count, el['avg-esgscore'], getRisk(el['avg-esgscore']))
  );
  // console.log(SORTING_SELECTING_TABLE);
  SORTING_SELECTING_TABLE = [...countryData, ...sectorData];
